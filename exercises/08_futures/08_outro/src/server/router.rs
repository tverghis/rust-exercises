...
use super::{
    schema::{NewTicketRequest, PatchTicketRequest},
    state::AppState,
};
...
pub fn router(state: Arc<Mutex<AppState>>) -> Router {
    Router::new()
        .route(
            "/tickets/:ticket_id",
            get(get_ticket_handler).patch(patch_ticket_handler),
        )
...
async fn post_ticket_handler(
    State(state): State<Arc<Mutex<AppState>>>,
    Json(body): Json<NewTicketRequest>,
) -> Result<Json<Ticket>, Json<CreateTicketError>> {
...
) -> Result<Json<Option<Ticket>>, Json<CreateTicketError>> {
    let mut state = state.lock().unwrap();
